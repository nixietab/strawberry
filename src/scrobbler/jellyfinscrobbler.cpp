...
JellyfinScrobbler::JellyfinScrobbler(const SharedPtr<ScrobblerSettingsService> settings, const SharedPtr<NetworkAccessManager> network, const SharedPtr<JellyfinService> service, QObject *parent)
  ...
  // The server idles out a session that has not received a check-in for a while, so periodically report the
  // playback progress while a Jellyfin song is playing to keep it visible on the server's dashboard.
...
void JellyfinScrobbler::UpdateNowPlaying(const Song &song) {
  ...
  if (song.source() != Song::Source::Jellyfin) return;
...
void JellyfinScrobbler::SendScrobbles() {
  ...
  qLog(Debug) << "JellyfinScrobbler: Submitting scrobble for" << song_playing_.artist() << song_playing_.title();
  submitted_ = false;
